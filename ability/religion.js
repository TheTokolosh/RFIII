...
gs.createReligionTypes = function () {
	...
	this.religionTypes.Wealth.onSet = function (character) {
		gs.getIndexListInBox(character.tileIndex.x - 1, character.tileIndex.y - 1, character.tileIndex.x + 2, character.tileIndex.y + 2).forEach(function (index) {
			if ((gs.isPassable(index) || gs.getChar(index)) && !gs.getItem(index) && !gs.isPit(index)) {
	...
	this.religionTypes.TheArcher.onTurn = function (character) {
		...
		if (util.frac() < (1 / 500)) {
			...
			gs.pc.inventory.addItem(Item.createItem(itemType.name));
		}
	};
	...
	this.religionTypes.Health.onTurn = function (character) {
		// Every 200 turns
		if (gs.pc.currentHp < gs.pc.maxHp / 2 && util.frac() < (1 / 200)) {
...
gs.createAltar = function (tileIndex, typeName) {
	...
	indexList = gs.getIndexListInBox(tileIndex.x - 1, tileIndex.y - 1, tileIndex.x + 2, tileIndex.y + 2);
	indexList = indexList.filter(index => util.frac() < 0.4);
