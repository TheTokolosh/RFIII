...
/*global LevelGeneratorUtils, vaultGenerator*/
...
SewerRoadsGenerator.prototype.generate = function (flags) {
	...
	// Initial Fill:
	this.placeTileSquare(0, 0, NUM_TILES_X, NUM_TILES_Y, gs.tileTypes.Floor);
	...
	this.placeTileSquare(1, 1, NUM_TILES_X - 1, NUM_TILES_Y - 1, gs.tileTypes.Wall, true);
...
SewerRoadsGenerator.prototype.placeRoads = function () {
	...
	// Full Vertical Road:
	possibleVerticalIndices = util.range(this.ROAD_SPACING, this.numTilesX - this.ROAD_SPACING, this.ROAD_SPACING);
	possibleVerticalIndices = util.randSubset(possibleVerticalIndices, 3);
	...
	possibleHorizontalIndices = util.range(this.ROAD_SPACING, this.numTilesY - this.ROAD_SPACING, this.ROAD_SPACING);
	possibleHorizontalIndices = util.randSubset(possibleHorizontalIndices, 3);
...
SewerRoadsGenerator.prototype.createBuildingArea = function (tileIndex) {
	...
	if (height > 16 || height > 2 * width) {
		this.buildingAreaList.push(LevelGeneratorUtils.createArea(tileIndex.x, tileIndex.y, endX, tileIndex.y + Math.floor(height / 2)));
	} 
	// Split Horizontal:
	else if (width > 16 || width > 2 * height) {
		this.buildingAreaList.push(LevelGeneratorUtils.createArea(tileIndex.x, tileIndex.y, tileIndex.x + Math.floor(width / 2), endY));
	}
	// Single Area:
	else {
		this.buildingAreaList.push(LevelGeneratorUtils.createArea(tileIndex.x, tileIndex.y, endX, endY));
	}
};
...
SewerRoadsGenerator.prototype.placeBuilding = function (area) {
	...
	if (util.frac() < 0.30) {
		this.placeTileSquare(area.startX + 1, area.startY + 1, area.endX - 1, area.endY - 1, gs.tileTypes.Wall);
	}
...
SewerRoadsGenerator.prototype.placeVaultRoom = function (area) {
	...
	vaultGenerator.placeVault(area.startTileIndex, vaultType);
};
...
SewerRoadsGenerator.prototype.placeToxicPool = function (area) {
	var innerBox = util.createBox(area.startX + 1, area.startY + 1, area.endX - 1, area.endY - 1);
	...
	if (util.boxType(innerBox) === 'SQUARE') {
		...
		this.placeTileSquare(innerBox.startX, innerBox.startY, innerBox.endX, innerBox.endY, gs.tileTypes.ToxicWaste);
	}
};
...
SewerRoadsGenerator.prototype.placeHallwayBuilding = function (area) {
	...
	// Walls:
	this.placeTileSquare(area.startX + 1, area.startY + 1, area.endX - 1, area.endY - 1, gs.tileTypes.Wall);
	this.placeTileSquare(area.startX + 2, area.startY + 2, area.endX - 2, area.endY - 2, gs.tileTypes.Floor);
...
SewerRoadsGenerator.prototype.placeSquareHallwayBuilding = function (area) {
	...
	// CrossRoad:
	this.placeTileSquare(area.centerX - 2, area.centerY - 1, area.centerX + 3, area.centerY + 2, gs.tileTypes.Floor);
	this.placeTileSquare(area.centerX - 1, area.centerY - 2, area.centerX + 2, area.centerY + 3, gs.tileTypes.Floor);
...
SewerRoadsGenerator.prototype.createRoomArea = function (startX, startY, endX, endY) {
	var area = LevelGeneratorUtils.createArea(startX, startY, endX, endY);
