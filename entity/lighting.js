...
// CREATE_REVIVE_SKELETON_EFFECT:
// ************************************************************************************************
gs.createReviveSkeletonEffect = function (tileIndex, callBack, context, callBackFrame = 5) {
	var pos = util.toPosition(tileIndex), callBackWrap;
	...
	// Anim Effect:
	let effect = gs.createAnimEffect({x: pos.x, y: pos.y + 15}, 'ReviveSkeletonBG');
	gs.createAnimEffect({x: pos.x, y: pos.y + 15}, 'ReviveSkeletonFG');
	gs.createLightCircle({x: pos.x, y: pos.y + 10}, '#ff00ff', 120, 50, '88');
			
	// Call Back:		
	effect.setOnFrame(callBackFrame, callBackWrap, context);	
};

// CREATE_SUMMON_EFFECT:
// ************************************************************************************************
gs.createSummonEffect = function (tileIndex, callBack, context, callBackFrame = 5) {
...
// CREATE_HEART_EFFECT:
// ************************************************************************************************
gs.createHeartEffect = function (tileIndex) {
	var pos = util.toPosition(tileIndex);
	...
	gs.createAnimEffect({x: pos.x, y: pos.y + 15}, 'Hearts');
	gs.createLightCircle({x: pos.x, y: pos.y + 10}, '#f06292', 60, 90, '66');
};
...
// ************************************************************************************************
gs.createPoisonEffect = function (tileIndex) {
	// Don't create offscreen:
	if (!gs.getTile(tileIndex).visible) {
		return;
	}
	...
	// Animation:
	gs.createAnimEffect({x: pos.x, y: pos.y + 10}, 'Poison');
	
	// Lighting: 
	gs.createLightCircle({x: pos.x, y: pos.y + 10}, '#aaff00', 60, 90, '66');
};
...
// ************************************************************************************************
gs.createShieldBreakEffect = function (tileIndex) {
	var pos = util.toPosition(tileIndex);
	pos.y -= 20;
	
	gs.createAnimEffect(pos, 'AltarAnimation');
	
	gs.createLightCircle(pos, '#fe74e3', 120, 30, '88');
};
...
gs.createAnimEffectTypes = function () {
	...
	// REVIVE_SKELETON_BACKGROUND:
	this.animEffectTypes.ReviveSkeletonBG = {
	...
	// REVIVE_SKELETON_FOREGROUND:
	this.animEffectTypes.ReviveSkeletonFG = {
		tileset: 'EffectsTileset',
		startFrame: 864,
		numFrames: 12,
		speed: 20,
	...
	// TORNADO_SUCK:
	this.animEffectTypes.TornadoSuck = {
		tileset: 'TornadoAnimation',
		startFrame: 16,
		numFrames: 8,
		speed: 50,
	};
	...
	// ALAR_ANIMATION:
	this.animEffectTypes.AltarAnimation = {
		tileset: 'Altar-Animation',
		startFrame: 0,
		numFrames: 12,
		speed: 50,
		pauseGame: false,
...
function AnimEffect (position, typeName, angle) {
	...
	if (this.type.backLayer) {
		group = gs.underObjectSpritesGroup;
	}
	else {
	...
	// Anim:
    this.sprite.animations.add('Anim', util.range(this.type.startFrame, this.type.startFrame + this.type.numFrames), this.type.speed, false);
...
AnimEffect.prototype.update = function () {
	if (this.animComplete()) {
		// Safety check:
		if (this.onFrameFunc && !this.onFrameHasComplete) {
...
function LightCircle (position, color, radius, life, startAlpha) {	
	...
	// Sprite:
	this.sprite = gs.createSprite(position.x, position.y, this.bmp, gs.lightSpritesGroup);
	this.sprite.anchor.setTo(0.5, 0.5);
